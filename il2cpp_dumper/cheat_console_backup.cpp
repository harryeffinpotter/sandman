...
static void scan_entities() {
...

    {
        ...
        for (int e = 0; e < entityCount; e++) {
            ...
            if (name.rfind("Sun", 0) == 0) continue;
            if (name.rfind("InventorySlot", 0) == 0) continue;
            if (name.rfind("TestColdEmitter", 0) == 0) continue;
            if (name.rfind("EXPEDITION_WALKER", 0) == 0) continue;
            if (name.rfind("game_navalMinesField", 0) == 0) continue;
            if (name.rfind("game_extractionPoint", 0) == 0) continue;
            if (name.rfind("game_destructibleDoor", 0) == 0) continue;
            if (name.rfind("game_sqrDoor", 0) == 0) continue;
        ...
        if (g_permaLockActive.load() && !g_permaLockName.empty() && !g_dupeMode.load()) {
            EnterCriticalSection(&g_itemsLock);
            bool found = false;
            for (auto& it : g_items) {
            ...
            if (!found) g_lockedEntityId.store(-1);
            LeaveCriticalSection(&g_itemsLock);
        }
    }
}
...
static void draw_ui() {
    ...
    for (size_t i = 0; i < g_items.size() && displayed < maxDisplay; i++) {
        ...
        printf("  %02d  | %-24.24s | %-7s | %-4s | %-6d\n",
               displayed + 1, item.name.c_str(), distBuf, src, item.entityId);
    ...
    printf("  [E] Dump entities   | [P] Probe context         \n");
    printf("  [Q] Quit                                        \n");
    printf("===================================================\n");
...
static void handle_input() {
    ...
    case 'P': {
        ...
        if (gcm) {
            ...
            if (!IsBadReadPtr(gcm, 0x90)) {
                pout("[HEX] GameContextModule (0x90 bytes):\n");
                uint8_t* p = (uint8_t*)gcm;
                for (int i = 0; i < 0x90; i += 16) {
                    pout("  %04X: ", i);
                    for (int j = 0; j < 16 && (i+j) < 0x90; j++) pout("%02X ", p[i+j]);
                    pout("\n");
                }

                for (int off = 0x10; off <= 0x88; off += 8) {
                    void* val = nullptr;
                    safe_read_ptr((void*)((uintptr_t)gcm + off), &val);
                    pout("  GCM+0x%02X: %p", off, val);
                    if (val && !IsBadReadPtr(val, 0x28)) {
                        int c1 = 0; safe_read_int((void*)((uintptr_t)val + 0x18), &c1);
                        int c2 = 0; safe_read_int((void*)((uintptr_t)val + 0x20), &c2);
                        pout(" [+0x18=%d, +0x20=%d]", c1, c2);
                    }
                    pout("\n");
                }
            }
            ...
            void* ctx = nullptr;
            safe_read_ptr((void*)((uintptr_t)gcm + 0x10), &ctx);
            if (ctx && !IsBadReadPtr(ctx, 0xB0)) {
                pout("\n[PROBE] Context at %p\n", ctx);
                pout("[HEX] Context (0xB0 bytes):\n");
                uint8_t* p = (uint8_t*)ctx;
                for (int i = 0; i < 0xB0; i += 16) {
                    pout("  %04X: ", i);
                    for (int j = 0; j < 16 && (i+j) < 0xB0; j++) pout("%02X ", p[i+j]);
                    pout("\n");
                }

                for (int off = 0x10; off <= 0xA0; off += 8) {
                    void* val = nullptr;
                    safe_read_ptr((void*)((uintptr_t)ctx + off), &val);
                    pout("  CTX+0x%02X: %p", off, val);
                    if (val && !IsBadReadPtr(val, 0x28)) {
                        int c1 = 0; safe_read_int((void*)((uintptr_t)val + 0x18), &c1);
                        int c2 = 0; safe_read_int((void*)((uintptr_t)val + 0x20), &c2);
                        pout(" [+0x18=%d, +0x20=%d]", c1, c2);
            ...
            pout("\n[PROBE] FindInteractSystem at %p\n", (void*)g_findInteractSystem);
            void* sys = (void*)g_findInteractSystem;
            if (sys && !IsBadReadPtr(sys, 0x60)) {
                pout("[HEX] System (0x60 bytes):\n");
                uint8_t* p = (uint8_t*)sys;
                for (int i = 0; i < 0x60; i += 16) {
                    pout("  %04X: ", i);
                    for (int j = 0; j < 16 && (i+j) < 0x60; j++) pout("%02X ", p[i+j]);
                    pout("\n");
                }
            }
        }

        pout("\n[INFO] Component indices: BP=%d Pos=%d IA=%d INA=%d Intrs=%d IT=%d Par=%d ItemT=%d\n",
             g_idx_blueprint, g_idx_position, g_idx_interactible,
             g_idx_interact_not_active, g_idx_interactions,
             g_idx_interact_target, g_idx_parent, g_idx_item_type);
...
static DWORD WINAPI worker_thread(LPVOID) {
    ...
    printf("[*] Component indices: BP=%d Pos=%d IA=%d INA=%d Intrs=%d IT=%d Par=%d ItemT=%d Id=%d\n",
           g_idx_blueprint, g_idx_position, g_idx_interactible,
           g_idx_interact_not_active, g_idx_interactions,
           g_idx_interact_target, g_idx_parent, g_idx_item_type, g_idx_id);
