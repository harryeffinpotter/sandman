...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    {
        if (bd->pd3dDevice->CreateVertexShader(g_vs_blob, sizeof(g_vs_blob), nullptr, &bd->pVertexShader) != S_OK)
            return false;
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, g_vs_blob, sizeof(g_vs_blob), &bd->pInputLayout) != S_OK)
            return false;
    ...
    {
        if (bd->pd3dDevice->CreatePixelShader(g_ps_blob, sizeof(g_ps_blob), nullptr, &bd->pPixelShader) != S_OK)
            return false;
    }
